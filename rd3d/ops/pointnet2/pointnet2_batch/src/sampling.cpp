...
int gather_points_bnc_wrapper_fast(int n, int m, int c, at::Tensor points_tensor, at::Tensor idx_tensor,
                                   at::Tensor out_tensor) {
    const float *points = points_tensor.data_ptr<float>();
    const int *idx = idx_tensor.data_ptr<int>();
    float *out = out_tensor.data_ptr<float>();

    gather_points_bnc_kernel_launcher_fast(n, m, c, points, idx, out);
    return 1;
}
//
//void gather_points_bnc_grad_wrapper_fast(int b, int c, int n, int m,
...
//
//    gather_points_grad_kernel_launcher_fast(b, c, n, m, grad_out, idx, grad_points);
//    return 1;
//}
...
int furthest_point_sampling_weights_wrapper(int b, int n, int m,
                                            at::Tensor xyz_tensor, at::Tensor weights_tensor, at::Tensor temp_tensor,
