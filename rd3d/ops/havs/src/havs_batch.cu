...
std::vector<at::Tensor> HAVSamplingBatchWrapper(at::Tensor &sources,  // [in]
    ...
    auto hash_tables_tensor = sources.new_empty({num_batch, num_hash}, TorchType<int>);
    auto dist_tables_tensor = sources.new_empty({num_batch, num_hash}, TorchType<float>);
    auto point_slots_tensor = sources.new_empty({num_batch, num_src}, TorchType<int>);
    auto point_dists_tensor = sources.new_empty({num_batch, num_src}, TorchType<float>);
