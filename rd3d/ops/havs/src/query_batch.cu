...
void QueryByPointHashingBatchLauncher(const uint32_t num_batch, const uint32_t num_src,
                                      ...
                                      uint64_t *queried_ids, uint32_t *num_queried,
...
void QueryByPointHashingBatchWrapper(at::Tensor &queries, at::Tensor &sources,
    ...
    const int64_t num_qry{queries.size(1)};
    const int64_t num_hash{hash_tables.size(1)};
    const int64_t num_nei{num_neighbours};
    ...
    QueryByPointHashingBatchLauncher(
            ...
            (uint64_t *) queried_ids.data_ptr(),
            (uint32_t *) num_queried.data_ptr()
    );
}
