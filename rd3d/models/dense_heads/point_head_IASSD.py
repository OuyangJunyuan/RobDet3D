...
class IASSD_Head(PointHeadTemplate):
    ...
    def __init__(self, num_class, input_channels, model_cfg, predict_boxes_when_training=False, **kwargs):
        ...
        # IA-SSD: input(512) -> (256 -> 256 -> 3) 可以认为IA-SSD把第一个512弄到SA里做aggregation了。
        # 3DSSD-SASA: input(1536) -> (512 -> 256) -> (128 -> 3)
    ...
    def assign_targets(self, input_dict):
        ...
        if extra_method is not None and extra_method.NAME == 'extend_gt':
            ...
            extend_gt_boxes = box_utils.enlarge_box3d_with_factor(
                gt_boxes.view(-1, gt_boxes.shape[-1]), factor=extra_method.EXTRA_FACTOR).view(batch_size, -1,
                                                                                              gt_boxes.shape[-1])
    ...
    def get_contextual_vote_loss(self, tb_dict=None):
        ...
        if len(center_origin_loss_box) != 0:
            ...
            if tb_dict is None:
                tb_dict = {}
            tb_dict.update({'center_origin_loss_reg': 0})
            center_origin_loss_box = 0
    ...
    def get_contextual_vote_loss_ver1(self, tb_dict=None):
        ...
        for cur_id in batch_idx.unique():
            ...
            for ins_idx in box_idxs_of_pts[batch_mask].unique():
                ...
                ins_vote_loss.append(F.smooth_l1_loss(centers_pred[batch_mask][ins_mask, 1:4],
                                                      center_box_labels[batch_mask][ins_mask, 0:3],
                                                      reduction='sum').unsqueeze(-1))
        ...
        for cur_id in batch_idx.unique():
            ...
            for ins_idx in box_idxs_of_pts[batch_mask].unique():
                ...
                ins_vote_loss.append(F.smooth_l1_loss(centers_pred[batch_mask][ins_mask, 1:4],
                                                      center_box_labels[batch_mask][ins_mask, 0:3],
                                                      reduction='sum').unsqueeze(-1))
                ins_mean_vote_loss.append(F.smooth_l1_loss(centers_pred[batch_mask][ins_mask, 1:4],
                                                           centers_pred[batch_mask][ins_mask, 1:4].mean(dim=0).repeat(
                                                               centers_pred[batch_mask][ins_mask, 1:4].shape[0], 1),
                                                           reduction='sum').unsqueeze(-1))
        ins_num = torch.cat(ins_num, dim=-1).float()
        ins_vote_loss = torch.cat(ins_vote_loss, dim=-1)
    ...
    def generate_center_ness_mask(self):
        ...
        offset_xyz_canical = common_utils.rotate_points_along_z(offset_xyz.unsqueeze(dim=1), -gt_boxes[:, 6]).squeeze(
            dim=1)
    ...
    def generate_sa_center_ness_mask(self):
        ...
        for i in range(len(sa_pos_mask)):
            ...
            offset_xyz_canical = common_utils.rotate_points_along_z(offset_xyz.unsqueeze(dim=1),
                                                                    -gt_boxes[:, 6]).squeeze(dim=1)
    ...
    def get_center_box_binori_layer_loss(self, tb_dict=None):
        ...
        # print(point_box_preds, point_box_labels)
