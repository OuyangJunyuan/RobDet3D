...
DATASET = dict(
    ...
    CLASS_NAMES=['Pedestrian'],  # modified
    POINT_CLOUD_RANGE=[0, -30, -30, 30, 30, 30],  # modified
    ...
    FOV_POINTS_ONLY=False,  # modified

    DATA_AUGMENTOR=dict(
        DISABLE_AUG_LIST=['placeholder'],
        AUG_CONFIG_LIST=[
            dict(NAME='gt_sampling',
                 USE_ROAD_PLANE=False,  # modified
                 DB_INFO_PATH=['kitti_dbinfos_train.pkl'],
                 PREPARE=dict(
                     filter_by_min_points=['Pedestrian:5'],
                     filter_by_difficulty=[-2]),  # modified
                 SAMPLE_GROUPS=['Pedestrian: 10'],  # modified
