...
MODEL = dict(
    ...
    BACKBONE_3D=dict(
        ...
        ENCODER=
        [
            dict(samplers=[dict(name='rvs', range=[0, 16384], sample=4096, pool='mean', max_pts_per_voxel=5,
                                coors_range=DATASET.POINT_CLOUD_RANGE, voxel=[0.339, 0.339, 0.296])],
...
RUN.workflows.train = [dict(state='train', split='train', epochs=70)] + \
