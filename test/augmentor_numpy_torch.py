...
class TestAugmentor(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.raw_points = data_dict["points"].copy()
        cls.raw_boxes = data_dict["gt_boxes"].copy()

    def setUp(self) -> None:
        self.points = self.data_dict["points"].copy()
        self.boxes = self.data_dict["gt_boxes"].copy()
    ...
    #     global_rotate = AUGMENTOR.from_cfg(dict(name='global_rotate', prob=1.0, range=[-0.78539816, 0.78539816]))
    #     global_rotate(self.data_dict)
    #
    # def test1_global_translate(self):
    #     global_translate = AUGMENTOR.from_cfg(dict(name='global_translate', prob=1.0, std=[2, 2, 0]))
    #     global_translate(self.data_dict)
    #
    # def test2_global_scale(self):
    #     global_scale = AUGMENTOR.from_cfg(dict(name='global_scale', prob=1.0, range=[0.95, 1.05]))
    ...
    #     global_sparsify = AUGMENTOR.from_cfg(dict(name='global_sparsify', prob=1.0, keep_ratio=0.8))
    ...
    def test999_compose(self):
        aus = AugmentorList([
            ...
            dict(name='global_scale', prob=1.0, range=[0.1, 2.0]),
            dict(name='global_flip', prob=1.0, axis=['x', 'y']),
            dict(name='global_sparsify', prob=1.0, keep_ratio=0.1),
            dict(name='frustum_sparsify', prob=1.0, direction=[0, 0], range=np.pi / 4, keep_ratio=0.2),
